...
# ---- Administration ----------------------------------------------------------
# Env vars surfaced on the admin screen. Values are NEVER returned — only
# whether each is configured — so keys can't leak through the API.
_MANAGED_ENV = [
    ("NVIDIA_REASONING_EXEC_API_KEY", "Executive reasoning model"),
    ("NVIDIA_REASONING_OPS_API_KEY", "Operations reasoning model"),
    ("NVIDIA_CODING_API_KEY", "Coding model"),
    ("NVIDIA_EMBED_API_KEY", "Embedding model (RAG)"),
    ("MAPTILER_API_KEY", "Map basemap tiles"),
    ("GROQ_API_KEY", "Offline / fallback LLM"),
]
...
def admin_snapshot() -> dict[str, Any]:
    def build() -> dict[str, Any]:
        import config
        from modules import store
        # AI providers from the real capability registry (capability → model)
        from ai.router import AI
        reg = AI.router.registry
        providers = []
        for cap in reg.configured_capabilities():
            spec = reg.resolve(cap)
            if spec is None:
                continue
            providers.append({
                "capability": cap,
                ...
                "model": spec.model or "—",
                "configured": bool(spec.model) and config.get_env(spec.api_key_env) is not None,
            })
        # API keys — presence only, never the value
        keys = [{
            "name": name, "purpose": purpose,
            "configured": config.get_env(name) is not None,
            "masked": "••••••••" if config.get_env(name) is not None else "not set",
        } for name, purpose in _MANAGED_ENV]
        audit = store.load_audit_log(limit=40)
        return {
            "providers": providers,
            "api_keys": keys,
            "audit": audit,
    ...
    return _safe(build, {"providers": [], "api_keys": [], "audit": [],
                         "users": _SEED_USERS, "roles": _SEED_ROLES,
                         "integrations": _SEED_INTEGRATIONS})


# Representative admin config (seed data — a real deployment binds these to an
# identity provider / RBAC store).
_SEED_USERS = [
    {"name": "Kishan P.", "email": "kishan@supchainmate.io", "role": "Owner", "status": "active"},
    {"name": "Ops Planner", "email": "planner@supchainmate.io", "role": "Planner", "status": "active"},
    {"name": "Finance Lead", "email": "finance@supchainmate.io", "role": "Approver", "status": "active"},
    {"name": "Warehouse Sup.", "email": "wh@supchainmate.io", "role": "Operator", "status": "invited"},
]
_SEED_ROLES = [
    {"role": "Owner", "view": True, "run": True, "approve": True, "admin": True},
    {"role": "Approver", "view": True, "run": True, "approve": True, "admin": False},
    {"role": "Planner", "view": True, "run": True, "approve": False, "admin": False},
    {"role": "Operator", "view": True, "run": False, "approve": False, "admin": False},
]
_SEED_INTEGRATIONS = [
    {"name": "SAP S/4HANA", "kind": "ERP", "status": "not_connected"},
    {"name": "Oracle Fusion", "kind": "ERP", "status": "not_connected"},
    {"name": "NetSuite", "kind": "ERP", "status": "not_connected"},
    {"name": "Manhattan WMS", "kind": "WMS", "status": "not_connected"},
]


# ---- Commercial Intelligence -------------------------------------------------
# Real order/SKU volumes with transparent margin assumptions (labelled). A real
# deployment feeds line-item revenue, cost, and discount data.
_AOV = 120.0            # modelled average order value ($)
_GROSS_MARGIN = 0.32    # modelled blended gross margin
_TARGET_MARGIN = 0.35   # target margin for the underpricing check
_DISCOUNT_RATE = 0.08   # modelled discount leakage
_RETURNS_RATE = 0.03    # modelled returns
...
def _commercial():
    import numpy as np
    import pandas as pd
    from modules import cost_audit, forecast, sku
    # Customer profitability by state (real order counts × modelled economics)
    ...
    by_state = merged.groupby("customer_state").size().reset_index(name="orders")
    by_state["revenue"] = by_state["orders"] * _AOV
    by_state["margin"] = by_state["revenue"] * _GROSS_MARGIN
    by_state["margin_pct"] = _GROSS_MARGIN * 100
    by_state = by_state.sort_values("revenue", ascending=False)
    segments = [{
        "segment": str(r["customer_state"]), "orders": int(r["orders"]),
        "revenue": round(float(r["revenue"]), 0), "margin": round(float(r["margin"]), 0),
        "margin_pct": round(float(r["margin_pct"]), 1),
    } for _, r in by_state.head(12).iterrows()]
    total_revenue = float(by_state["revenue"].sum())

    # Revenue leakage — real freight overspend from the audit + modelled discount
    leakage_freight = 0.0
    try:
        orders_full = forecast.load_orders()
        from modules import control_tower
        shipments = control_tower.prepare_shipments(control_tower.assign_demo_carriers(orders_full))
        audit = cost_audit.run_audit(shipments)
        leakage_freight = float((audit or {}).get("flagged_value", 0.0))
    except Exception:  # noqa: BLE001
        pass
    leakage_discount = total_revenue * _DISCOUNT_RATE * 0.25  # recoverable slice
    leakage_total = leakage_freight + leakage_discount

    # Underpriced SKUs → recommended pricing (real unit prices from profiles)
    tickets = []
    try:
        o = forecast.load_orders()
        if "order_date" not in o.columns:
            o = o.assign(order_date=o["order_purchase_timestamp"])
        o = o.assign(quantity=1.0)
        prof = sku.sku_demand_profiles(sku.assign_demo_skus(o))
        for _, r in prof.iterrows():
            price = float(r.get("Unit Price") or 0)
            if price <= 0:
                continue
            unit_cost = price * (1 - _GROSS_MARGIN)             # modelled cost basis
            cur_margin = (price - unit_cost) / price
            if cur_margin < _TARGET_MARGIN:
                rec_price = unit_cost / (1 - _TARGET_MARGIN)    # price to hit target
                annual_units = float(r.get("Avg Daily") or 0) * 365
                tickets.append({
                    "sku": str(r["SKU"]),
                    "current_price": round(price, 2),
                    "recommended_price": round(rec_price, 2),
                    "uplift_pct": round((rec_price / price - 1) * 100, 1),
                    "current_margin_pct": round(cur_margin * 100, 1),
                    "annual_impact": round((rec_price - price) * annual_units, 0),
                })
        tickets.sort(key=lambda t: t["annual_impact"], reverse=True)
    except Exception:  # noqa: BLE001
        pass

    # Margin waterfall (network totals)
    discounts = total_revenue * _DISCOUNT_RATE
    returns = total_revenue * _RETURNS_RATE
    cogs = total_revenue * (1 - _GROSS_MARGIN)
    net = total_revenue - discounts - returns - leakage_freight - cogs
    waterfall = [
        {"label": "Gross revenue", "value": round(total_revenue, 0), "kind": "start"},
        {"label": "Discounts", "value": round(-discounts, 0), "kind": "neg"},
        {"label": "Returns", "value": round(-returns, 0), "kind": "neg"},
        {"label": "Freight leakage", "value": round(-leakage_freight, 0), "kind": "neg"},
        {"label": "COGS", "value": round(-cogs, 0), "kind": "neg"},
        {"label": "Net margin", "value": round(net, 0), "kind": "end"},
    ]
    return {
        "segments": segments,
        "kpis": {
            ...
            "net_margin_pct": round(net / total_revenue * 100, 1) if total_revenue else 0,
            "revenue_leakage": round(leakage_total, 0),
            "underpriced_skus": len(tickets),
            "repricing_upside": round(sum(t["annual_impact"] for t in tickets), 0),
        },
        "leakage": {"freight": round(leakage_freight, 0), "discount": round(leakage_discount, 0), "total": round(leakage_total, 0)},
        "waterfall": waterfall,
        "tickets": tickets[:12],
        "assumptions": {"aov": _AOV, "gross_margin_pct": _GROSS_MARGIN * 100,
                        "target_margin_pct": _TARGET_MARGIN * 100},
    }


def commercial_snapshot() -> dict[str, Any]:
    return _safe(lambda: _commercial(), {
        "segments": [], "kpis": {}, "leakage": {}, "waterfall": [], "tickets": [], "assumptions": {}})


def repricing_email(sku: str) -> dict[str, Any]:
    """Draft a repricing outreach email (AI when reachable, template otherwise)."""
    def build() -> dict[str, Any]:
        data = _commercial()
        ticket = next((t for t in data["tickets"] if t["sku"] == sku), None)
        if ticket is None:
            raise KeyError(sku)
        from ai import AI
        prompt = (
            f"Draft a concise, professional internal email proposing a price change for "
            f"SKU {ticket['sku']}. Current price ${ticket['current_price']}, recommended "
            f"${ticket['recommended_price']} (+{ticket['uplift_pct']}%), current margin "
            f"{ticket['current_margin_pct']}%, estimated annual upside "
            f"${ticket['annual_impact']:,.0f}. Justify by margin recovery to a 35% target. "
            f"4-6 sentences, ready to send.")
        resp = AI.ask("reasoning.operations", task="repricing_email", user=prompt)
        body = getattr(resp, "text", None) or _email_template(ticket)
        return {"sku": sku, "subject": f"Proposed repricing — {ticket['sku']} (+{ticket['uplift_pct']}%)",
                "body": body, "ticket": ticket}
    return _safe(build, {"sku": sku, "subject": f"Proposed repricing — {sku}",
                         "body": "Draft unavailable — AI provider not reachable.", "ticket": None})


def _email_template(t: dict) -> str:
    return (
        f"Hi team,\n\nRecommend repricing {t['sku']} from ${t['current_price']} to "
        f"${t['recommended_price']} (+{t['uplift_pct']}%). Current margin sits at "
        f"{t['current_margin_pct']}%, below our 35% target; this change restores it and adds an "
        f"estimated ${t['annual_impact']:,.0f} in annual margin at current volume.\n\n"
        f"The uplift is modest relative to category pricing and low elasticity on this line. "
        f"Requesting approval to roll out next cycle.\n\nThanks")
